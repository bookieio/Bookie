    ...
    ns.TagControl = Y.Base.create('bookie-tagcontrol', Y.Widget, [], {
        ...
        _add_tag: function () {
            ...
            var input = this.ui.one('input'),
                ...
                new_tag = Y.Node.create(this.tpl.tag);

            new_tag.addClass(this.getClassName('item'));
            new_tag.setContent(current_text);

            // add a new li element before the input one
            var parent = input.get('parentNode');
            parent.get('parentNode').insertBefore(new_tag, parent);
